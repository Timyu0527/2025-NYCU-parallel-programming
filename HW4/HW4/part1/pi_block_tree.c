#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>
#include <sys/types.h>
#include <time.h>
#include <unistd.h>

int main(int argc, char **argv)
{
    // --- DON'T TOUCH ---
    MPI_Init(&argc, &argv);
    double start_time = MPI_Wtime();
    double pi_result;
    long long int tosses = atoi(argv[1]);
    int world_rank, world_size;
    // ---

    // TODO: MPI init
    int SEED = 12345;
    long long count = 0;

    MPI_Comm_size(MPI_COMM_WORLD, &world_size);
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);

    long long local_tosses = tosses / world_size;
    long long remain = tosses % world_size;
    int seed = (world_rank + 1) * SEED;

    srand(seed);

    // TODO: binary tree redunction
    long long local_count = 0;

    if(world_rank < remain) local_tosses++;

    for(int i = 0; i < local_tosses; ++i){
        double x = (2.0 * rand_r(&seed) / RAND_MAX) - 1.0;
        double y = (2.0 * rand_r(&seed) / RAND_MAX) - 1.0;
        double distance_squared = x * x + y * y;
        if (distance_squared <= 1) local_count++;
    }
    // printf("local count: %lld\n", local_count);

    int k = 2, level = 0, t = world_size;
    while(t > 0) {
        level++;
        t >>= 1;
    }

    MPI_Status status;
    for(int i = 0; i < level; ++i){
        int dest = (world_rank % k == 0) ? -1 : world_rank - (k / 2);
        int src = (world_rank % k == 0) ? world_rank + (k / 2) : -1;
        if(dest != -1){
            MPI_Send(&local_count, 1, MPI_LONG_LONG, dest, 0, MPI_COMM_WORLD);
            // printf("%d to %d\n", world_rank, dest);
            break;
        }
        else if(src != -1 && src < world_size){
            int tmp;
            MPI_Recv(&tmp, 1, MPI_LONG_LONG, src, 0, MPI_COMM_WORLD, &status);
            local_count += tmp;
            // printf("%d from %d value %lld\n", world_rank, src, local_count);
        }
        k <<= 1;
    }

    if (world_rank == 0)
    {
        // TODO: PI result
        count = local_count;
        double pi_result = 4 * count / ((double)tosses);

        // --- DON'T TOUCH ---
        double end_time = MPI_Wtime();
        printf("%lf\n", pi_result);
        printf("MPI running time: %lf Seconds\n", end_time - start_time);
        // ---
    }

    MPI_Finalize();
    return 0;
}
