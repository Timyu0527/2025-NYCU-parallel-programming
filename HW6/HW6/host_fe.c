...
void host_fe(int filterWidth, float *filter, int imageHeight, int imageWidth,
            ...
            cl_context *context, cl_program *program)
{
    ...
    static cl_mem cached_input = NULL;
    ...
    if(cached_input == NULL){
        cached_input = clCreateBuffer(*context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, imageSize * sizeof(float), inputImage, &status);
        CHECK(status, "clCreateBuffer");
    }


    cl_mem filterMemObj = clCreateBuffer(*context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, filterSize * sizeof(float), filter, &status);
    CHECK(status, "clCreateBuffer");
    ...
    clSetKernelArg(kernel, 0, sizeof(cl_mem), (void *)&cached_input);
    ...
    size_t tile_size = (BLOCK + filterWidth - 1) * (BLOCK + filterWidth - 1) * sizeof(float);
    clSetKernelArg(kernel, 6, tile_size, NULL);


    // Execute the OpenCL kernel on the list
    size_t globalWorkSize[2] = {
        ((imageWidth  + BLOCK - 1) / BLOCK) * BLOCK,
        ((imageHeight + BLOCK - 1) / BLOCK) * BLOCK
    };
    size_t localWorkSize[2] = {BLOCK, BLOCK}; // Note : global_work_size must be divisible by local_work_size
