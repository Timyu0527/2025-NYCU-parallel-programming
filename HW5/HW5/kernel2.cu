...
__global__ void mandel_kernel(float lower_x, float lower_y, float step_x, float step_y, int count, int pitch, int *output)
{
    ...
    int *row = (int *)((char*)output + thready * pitch);
    row[threadx] = i;
}
             ...
             int max_iterations)
{
    ...
    mandel_kernel<<<blocks, threads>>>(lower_x, lower_y, step_x, step_y, max_iterations, pitch, output);
